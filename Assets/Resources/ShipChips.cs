...
internal static class ShipDefs
{
    public static ShipDefinition BasicShip()
    {
        ...
        var hardpointDefs = new List<HardpointDefinition>
        {
            new HardpointDefinition(HardpointType.LazerTurret, ComponentViewType.Turret, 0, Vector2.zero),
            new HardpointDefinition(HardpointType.BasicScanner, ComponentViewType.Scanner, 1, new Vector2(0.1f, 0))
        };
        var componentDefs = new List<ComponentDefinition>
        {
            new ComponentDefinition(ComponentType.BasicTurret, 0, new List<int> {1}),
            new ComponentDefinition(ComponentType.BasicScanner, 1, new List<int> {0, 1}),
    ...
    {
        const ShipViewType shipViewType = ShipViewType.BasicShip;
        var wireDefs = new List<WireDefinition>();
        var hardpointDefs = new List<HardpointDefinition>();
        var componentDefs = new List<ComponentDefinition>();
...
//        new AAddB(ship, distThrustWire, orbitThrustWire, netThrustWire);
//
//        //Bearing calculations
//        new AnalogueConstant(ship, 90.0f, orbitBearing);
//        new AAddB(ship, orbitBearing, bearingWire, netBearingWire);
//
